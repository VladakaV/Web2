...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    if (!empty($_SESSION['uid'])) {
        ...
        $values['birth'] = strip_tags($user[0]['birth']);
        $values['radio_sex'] = strip_tags($user[0]['radio_sex']);
        
        $sth = $db->prepare("SELECT id_lang FROM users_and_languages where id_user = $uid");
        $sth->execute();
        ...
        $values['checkbox_agree'] = strip_tags($user[0]['checkbox_agree']);
        include('form.php');
